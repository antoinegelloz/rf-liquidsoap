...
- Triggered by `workflow_dispatch` (manual trigger) on a branch or tag.
- Builds artifacts for the branch or tag using `make artifact VERSION=${{ github.ref_name }}` and creates/updates a draft pre-release.
- The workflow attaches a zip and an md5 into a draft pre-release here: https://github.com/radiofrance/rf-liquidsoap/releases.
